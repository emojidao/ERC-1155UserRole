...
abstract contract BaseDoNFT is OwnableContract,ERC721Upgradeable,ReentrancyGuardUpgradeable,IBaseDoNFT {
    ...
    function _BaseDoNFT_init(string memory name_, string memory symbol_,address address_,address market_) internal onlyInitializing {
    ...
    function onlyApprovedOrOwner(address spender,address nftAddress,uint256 tokenId) internal view returns(bool){
        ...
        return (spender == _owner || ERC721(nftAddress).getApproved(tokenId) == spender || ERC721(nftAddress).isApprovedForAll(_owner, spender));
    }
    ...
    function checkIn(address to,uint256 tokenId,uint256 durationId) public virtual{
        ...
        emit CheckIn(msg.sender,to,tokenId,durationId,info.oid,duration.end);
    }
...
contract Royalty is IRoyalty {
    ...
    function setBeneficiary(address payable beneficiary_) virtual public onlyRoyaltyAdmin{
...
contract CheckInMgr is OwnableContract{
    ...
    function resetExpiredTo(uint256[] memory ids,address to) public virtual onlyAdmin{
    ...
    function setUser(uint256 oid,address to,uint64 expiredAt) internal virtual{
...
abstract contract VipDoNFT is WrapDoNFT,Royalty,CheckInMgr{
    function initialize(string memory name_, string memory symbol_,address nftAddress_,address market_,address royaltyAdmin_) public virtual initializer {
        super._BaseDoNFT_init(name_,symbol_,nftAddress_,market_);
    ...
    function tokenURI(uint256 tokenId) public view virtual override returns (string memory) {
        ...
        if(isXNft(tokenId)){
            name_ = string(abi.encodePacked("w-",originalSymbol));
            type_value = isWrap() ? "wNFT" : "vNFT";
        }else{
            name_ = string(abi.encodePacked("do-",originalSymbol));
            type_value = "doNFT";
        }
        
        return DoubleSVG.genTokenURI(tokenId, name_, type_value, start_time, end_time);
    }
...
contract DclDoNFT is VipDoNFT{
    ...
    function getUser(uint256 originalNftId) public override view returns(address){
        return DCL(oNftAddress).updateOperator(originalNftId);
    }

    function setUser(uint256 oid,address to,uint64 expiredAt) internal override virtual{
